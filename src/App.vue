          ...
          <div class="col-sm-3">
            ...
            >
              <option
                ...
                :key="item._id"
                :value="item.episode"
                :selected="item.episode == selected"
              >{{ item.episode }}</option>
            </select>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async getList() {
      const { data } = await axios.get(`${this.apiEndpoint}/list`)
      this.episodes = data
      this.selected = data[0].episode
    },
    ...
    play(id) {
      const { $refs: { audio } } = this
      audio.src = `${process.env.VUE_APP_AUDIO_SRC}/${id}.mp3`
      audio.play()
    }
  }
}
